...
main ()
{
    ...
    Struct Stat stat;
    result = zoo_get (zh, path, 0, start_graph_database, sizeof (int), &stat);
    ...
    if (load_graph == 1) {
        printf ("\nIt apears that the database has already started");
        return 0;
    }
    load_graph = 1;
    ...
    if (result == ZOK) {
        printf ("\nPlatanos %s has been signaled to start the database",
                octopus);
