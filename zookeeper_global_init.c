...
result=zoo_create(zh,path,(const char *)&replication,sizeof(int),&ZOO_OPEN_ACL_UNSAFE,0,NULL,0);
if(result!=ZOK){
printf("\nError");
return 1;
}
...
result=zoo_create(zh,path,(const char *)&interval_size,sizeof(unsigned long),&ZOO_OPEN_ACL_UNSAFE,0,NULL,0);
if(result!=ZOK){
printf("\nError");
return 1;
}
